...
#ifdef TFT_MOD 
#ifndef USE_ADAFRUIT_TFT_LIB


// NOTE: This demo should run, but may look odd on 128x128 LCD (vs 128x160)

void loop_tft_eSPI(void)
{
  
  Serial.print(F(__DATE__ " " __TIME__ " - Flash=0x"));
  Serial.println(F("Benchmark                Time (microseconds)"));  uint32_t usecHaD = testHaD();
  ...
  vTaskDelay(pdMS_TO_TICKS(15 * 1000L ));
}
...
uint32_t testHaD()
{
  ...
  tft.print(F("         Xark"));
  vTaskDelay(pdMS_TO_TICKS( 3 * 1000L));  
  return t;
}
...
void Task3code( void * pvParameters ){
  int counter1 =0;
  Serial.print("Task3 (TFT_eSPI) starting on core ");  Serial.println(xPortGetCoreID()); 

  for(;;){
     if( Task3_flag == true) { 
         // execute TFT_eSPI loop 
         loop_tft_eSPI();
