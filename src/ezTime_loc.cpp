...
namespace {
  ...
  String debugLevelString(const ezDebugLevel_t level) {
    switch (level) {
      ...
      case ERROR: return  F("debugLevelString: ERROR");
...
namespace ezt {
  ...
  void setDebug(const ezDebugLevel_t level, Print &device) { 
    ...
    info("ezTime debug level set to ");
  ...
  void events() {
    if (!_initialised) {  // only executed on init
      for (uint8_t n = 0; n < MAX_EVENTS; n++) _events[n] = { 0, NULL };
      #ifdef EZTIME_NETWORK_ENABLE
        if (_ntp_interval) update_NTP_GPS(); // Start the cycle of update_NTP_GPS running and then setting an event for its next run
      #endif
      _initialised = true;
      };
    ...
    for (uint8_t n = 0; n < MAX_EVENTS; n++) {
      if (_events[n].function && ( nowUTC(false) >= _events[n].time)) {
        debug("Running event (#"); debug(n + 1); debug(") set for "); debugln(UTC.dateTime(_events[n].time));
        void (*tmp)() = _events[n].function;
        if(ezTime_debug) {debugA("\nezTime_events: Running event (#"); debugA(") set for "); debugA("%s",UTC.dateTime(_events[n].time));};
        _events[n].time = 0 ; // Delete any events pointing here, in case called manually
        _events[n] = { 0, NULL };   // reset the event
//        Serial.println(" ==================>  before func ");
        (tmp)();            // execute the function
//        Serial.println(" ==================>  after func ");
//removed        if (_ntp_interval) _events[n].time = nowUTC(false) + _ntp_interval ;
      }
    }
    yield();
  }
...
//********************************************************************
bool queryGPS(time_t &GPS_t,  unsigned long  &GPS_measured_at ){  // return last GPS_PPS rising edge time and millis ; if GPS is not valid return false, either true
  vTaskDelay(pdMS_TO_TICKS(500));  // wait 0,5 sec to be safe 
  bool boolCheck = true; 
  int sats=gps.satellites.value();  
  struct tm mytm;
  // GPS_sync_time is taken from ISR and is reported as now() at Interrupt service routine execution time
  if((gps_debug)||(ezTime_debug)) debugA("GPS_sync_time from ISR = %ld - GPS_rising_pps_millis = %d \r",GPS_sync_time, GPS_rising_pps_millis);
  char date_string[100];   // calculate the virtual time of the seconds boundary based non time strings coming from GPS
  if((gps_debug)||(ezTime_debug)) debugA( "GPS position fix found: date = %d/%d/%d %d:%d:%d\r" ,gps.date.day(),gps.date.month(),gps.date.year(), gps.time.hour(),gps.time.minute(),gps.time.second());
  sprintf(date_string, "%d/%d/%d %d:%d:%d" ,gps.date.day(),gps.date.month(),gps.date.year(), gps.time.hour(),gps.time.minute(),gps.time.second());
  strptime(date_string ,"%d/%m/%Y %H:%M:%S",&mytm);
  GPS_sync_time = mktime(&mytm) + 7200 + 1;   // this is the virtual time with 1 sec granulariry

  if(gps.satellites.value() > 3 ){
     if((gps_debug)||(ezTime_debug)) debugA("GPS_sync_time = %d - GPS_rising_pps_millis = %d - gps.satellites.value = %d \r",  GPS_sync_time, GPS_rising_pps_millis, gps.satellites.value());  // time & millis of last GPS_PPS rising edge
     GPS_t =  GPS_sync_time;  
     GPS_measured_at = GPS_rising_pps_millis;
     return boolCheck ;
     }
  else{
     if((gps_debug)||(ezTime_debug)) debugA("GPS position fix is still invalid\r");
     if((gps_debug)||(ezTime_debug)) debugA("GPS_sync_time = %d - GPS_rising_pps_millis = %d - gps.satellites.value = %d \r",  GPS_sync_time, GPS_rising_pps_millis, gps.satellites.value());  // time & millis of last GPS_PPS rising edge
...
void update_NTP_GPS() {   // updates NTP_time_status and GPS_time_status
    deleteEvent(update_NTP_GPS); // Delete any events pointing here, in case called manually
    time_t t=0; time_t t_NTP; time_t t_GPS;
    unsigned long measured_at; unsigned long measured_at_NTP; unsigned long measured_at_GPS;
    bool ezTime_debug_old = ezTime_debug;  // ezTime_debug=1 ;
    if(ezTime_debug) debugA("update_NTP_GPS......\r");
    bool query_ret ; bool query_ret_NTP ; bool query_ret_GPS ;
    if(ezTime_debug) debugA("NTPServer_eztime = %s  ntpServer = %s   LocalTimeNow before update = %d/%d %s",_ntp_server.c_str(), ntpServer.c_str() , now(),millis(), ( UTC.dateTime(now(), "d-M-y H:i:s.v T\n")).c_str()  ); 
    query_ret_NTP = queryNTP(_ntp_server, t_NTP, measured_at_NTP);    
    query_ret_GPS = queryGPS(t_GPS, measured_at_GPS );
                
    if((op_status == 3)&& query_ret_GPS ) {
        t=t_GPS ; 
        measured_at = measured_at_GPS ;
       }
    else if((op_status < 3)&& query_ret_NTP ) {
        t=t_NTP ;
    ...
    if ((query_ret_NTP || query_ret_GPS ) && t) {
       ...
       if(ezTime_debug) debugA("query_ret_NTP = %d -  query_ret_GPS = %d - time/measured_at=%d/%d\r",query_ret_NTP, query_ret_GPS , t, measured_at);
       int32_t correction = ( (t - _last_sync_time) * 1000 ) - ( measured_at - _last_sync_millis );
       if (abs(correction) < 900 ) time_jitter = (time_jitter + abs(correction))/ 2 ; 
//     if (abs(correction) < 900 ) { time_jitter = (time_jitter + abs(correction))/ 2 ; } else {time_jitter= -1 ; } ;
       if(ezTime_debug) debugA("===> correction=%ld - time_jitter=%ld\r", correction, time_jitter);
       if((_last_sync_time )&& (op_status == 3)){   // GPS time fix must be 1sec jump tolerant...
          if(ezTime_debug) debugA("before NTP_GPS time fixing: time/correction = %d/%d\r", t, correction);
          if (correction > 900) {
             t = t -1 ;
             correction = ( (t - _last_sync_time) * 1000 ) - ( measured_at - _last_sync_millis );
             if(ezTime_debug) debugA("after NTP_GPS time fixing: time/correction = %d/%d\r", t, correction);
             }
          else if (correction < -900) {
             t = t + 1 ;
             correction = ( (t - _last_sync_time) * 1000 ) - ( measured_at - _last_sync_millis );
             if(ezTime_debug) debugA("after NTP_GPS time fixing: time/correction = %d/%d\r", t, correction);
            };
       ...
       if(query_ret_GPS) { GPS_time_status = timeSet;} else {GPS_time_status = timeNotSet;};
       if(query_ret_NTP) { NTP_time_status = timeSet;} else {NTP_time_status = timeNotSet;};
       
       if(ezTime_debug) debugA("Received time: %s\r",(UTC.dateTime(t, "d-M-y H:i:s.v T")).c_str());
       if (_time_status != timeNotSet) {
         String res;

          if (!correction) {
             res= String("spot on\r");
             } 
          else {
            res=String(abs(correction));
            if (correction > 0) {
               res = res  + String(" ms fast");
               } 
            else {
               res = res  + String(" ms slow");
               };  // end of correction >0 
            };   //  end of correction 
        if(ezTime_debug) debugA("====> internal clock was %s \r", res.c_str());
            
          } 
       else {
          if(ezTime_debug)  debugA("\r");
          };  // end of != timeNotSet
                 
       if(ezTime_debug)debugA("(NTP_GPS_OK) LocalTimeNow after update = %d/%d - GPS_time_status = %d - NTP_time_status = %d at %s",now(),millis(), GPS_time_status, NTP_time_status, (UTC.dateTime(t, "d-M-y H:i:s.v T")).c_str()); 
       } // query ret with _time_status = timeSet
  else {  // both the time sync sources are not available... try to see if it will be recovered in a short time...
          // declare timeNotSet due to failed sync source
          _time_status = timeNotSet;
          if(query_ret_GPS) { GPS_time_status = timeSet;} else {GPS_time_status = timeNotSet;};
          if(query_ret_NTP) { NTP_time_status = timeSet;} else {NTP_time_status = timeNotSet;};
                 
          if(ezTime_debug){ 
             debugA("(NTP_GPS_FAILED) LocalTimeNow after update = %d/%d - GPS_time_status = %d - NTP_time_status = %d at ",now(),millis(), GPS_time_status, NTP_time_status); 
             debugA("%s",UTC.dateTime(now(), "d-M-y H:i:s.v T\r"));
             };             
...
bool queryNTP(const String server, time_t &t, unsigned long &measured_at) {
      if(ezTime_debug) debugA("quering server = %s\r", server.c_str()); 

      #ifndef EZTIME_ETHERNET
      if(! standalone){
        if (WiFi.status() != WL_CONNECTED) { 
          triggerError(NO_NETWORK); 
          if(ezTime_debug) debugA("NO_NETWORK\r\n"); 
          return false; 
          }
        };  
        WiFiUDP udp;
      ...
      if( standalone ) return false;  // if neither wifi nor ethernet is available just fail..
     ...
     //   debugA("parsePacket = %d %s", NTP_TIMEOUT, (UTC.dateTime(t, "d-M-y H:i:s.v T")).c_str());
        delay (1);
        // vTaskDelay(pdMS_TO_TICKS(20));
        if (millis() - started > NTP_TIMEOUT) {
          udp.stop(); 
          triggerError(TIMEOUT); 
          if(ezTime_debug) debugA("query TIMEOUT...\r"); 
          return false;
        }
      }
      ...
      if ((buffer[1] < 1) or (buffer[1] > 15) or (reftsSec == 0) or (rcvtsSec == 0) or (rcvtsSec > secsSince1900)) {
        ...
        if(ezTime_debug) debugA("received INVALID_DATA\r"); 
        return false;
      }

      // Set the t and measured_at variables that were passed by reference
      uint32_t done = millis();
      if(ezTime_debug) debugA("queryNTP: success (round trip %ld ms)\r", done - started); 
...
//    if(ezTime_debug) debugA("query RETURN_TRUE\r");         
      return true;
    }

    void setInterval(const uint16_t seconds /* = 0 */) { 
      deleteEvent(update_NTP_GPS);
      _ntp_interval = seconds;
      if (seconds) UTC.setEvent(update_NTP_GPS, nowUTC(false) + _ntp_interval);
    }

    void setServer(const String ntp_server /* = NTP_SERVER */) { _ntp_server = ntp_server; ntpServer = ntp_server.c_str(); }
    ...
    bool waitForSync(const uint16_t timeout /* = 0 */) {
      ...
      if(! standalone){  
        if (WiFi.status() != WL_CONNECTED) {
          if(ezTime_debug) info(F("Waiting for WiFi ... "));
          while (WiFi.status() != WL_CONNECTED) {
            if ( timeout && (millis() - start) / 1000 > timeout ) { triggerError(TIMEOUT); return false;};
            events();
            vTaskDelay(pdMS_TO_TICKS(25));
          }
          if(ezTime_debug) infoln(F("connected"));
        }
      };  
      #endif

      if (_time_status != timeSet) {
        if(ezTime_debug)  infoln(F("Waiting for time sync"));
        while (_time_status != timeSet) {
          if ( timeout && (millis() - start) / 1000 > timeout ) { triggerError(TIMEOUT); return false;};
          vTaskDelay(pdMS_TO_TICKS(250));
          events();
        }
        if(ezTime_debug)  infoln(F("Time is in sync"));
      }
      return true;
    }
...
String Timezone::getPosix() { return _posix; }
  ...
  bool Timezone::setLocation(const String location /* = "GeoIP" */) {
    ...
    if(ezTime_debug)  info(F("Timezone lookup for: "));
    if(ezTime_debug)  info(location);
    if(ezTime_debug)  info(F(" ... "));
    if (_locked_to_UTC) { triggerError(LOCKED_TO_UTC); return false; }
    ...
    udp.stop();
    if(ezTime_debug)  info(F("(round-trip "));
    if(ezTime_debug) info(millis() - started);
    if(ezTime_debug) info(F(" ms)  "));
    ...
    if (recv.substring(0,3) == "OK ") {
      ...
      if(ezTime_debug) infoln(F("success."));
      if(ezTime_debug) info(F("  Olson: ")); infoln(_olson);
      if(ezTime_debug) info(F("  Posix: ")); infoln(_posix);
  ...
  String Timezone::getOlsen() {
    ...
    bool Timezone::writeCache(String &str) {
      ...
      if (year() >= 2018) months_since_jan_2018 = (year(LAST_READ) - 2018) * 12 + month(LAST_READ) - 1;
        ...
        if(ezTime_debug) info(F("Caching timezone data  "));
        ...
        if(ezTime_debug) infoln(F("Caching timezone data"));
        Preferences prefs;
        prefs.begin(_nvs_name.c_str(), false);
    ...
    bool Timezone::readCache(String &olson, String &posix, uint8_t &months_since_jan_2018) {
        ...
        if(ezTime_debug)  info(F("Cache read. Olson: ")); info(olson); info (F("  Posix: ")); infoln(posix);
        eepromEnd();
        return true;
      #endif            
...
uint8_t Timezone::setEvent(void (*function)(), time_t t /* = TIME_NOW */, const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
  ...
  for (uint8_t n = 0; n < MAX_EVENTS; n++) {
    if (!_events[n].function) {
      ...
      _events[n].time = t;
  //////    debug("Set event (#%d ) to trigger on: %s\r" , n + 1 , (UTC.dateTime(t)).c_str());;
      return n + 1;
    }
  }
...
String Timezone::dateTime(time_t t, const ezLocalOrUTC_t local_or_utc, const String format /* = DEFAULT_TIMEFORMAT */) {
  ...
  if (t == TIME_NOW || t == LAST_READ || local_or_utc == UTC_TIME) {
    // in these cases we actually want tzTime to transform the time for us
    ...
    // when receiving a local time we don't want to transform the timestamp
...
void ezTime_setup() {
   Serial.println("init ezTime synchronization with GPS/NTP: this check takes 5 or more seconds");
   Serial.printf("ezTime_setup-GPS: no_gps = %d - has_GPS = %d - standalone = %d - has_DS3231 = %d\r",no_gps, has_GPS,  standalone, has_DS3231);
   GPS_time_status = timeNotSet; NTP_time_status = timeNotSet;
   

   int cntr=10;  // try to check if GPS is available
   while ( ( GPS_time_status != 2 ) && (cntr-- >0) && (! no_gps) && has_GPS ){  // 2--> timeSet 
     if(ezTime_debug) debugA("ezTime_setup-GPS: ======(%d)======> try to check if a valid GPS fix is available...\r",cntr);
         gps_loop();  // go trough GPS NMEA sentences acquisition 
     vTaskDelay(pdMS_TO_TICKS(1000));
 //        updateGPS();
     if(ezTime_debug) debugA("   ezTime_setup-GPS: after events() GPS_time_status = %d \r",GPS_time_status);
     };
   if((cntr > 0) && (GPS_time_status == 2)) debugA("ezTime_setup-GPS: cntr = %d - GPS fix available: GPS_time_status = %d - gps.satellites.value = %d\r", cntr, GPS_time_status, gps.satellites.value() );

   cntr=10;  // try to check if NTP is available
   while (( NTP_time_status != 2)&& (cntr-- >0) && (! standalone ) ){  // 2--> timeSet 
     if(ezTime_debug)debugA("   ezTime_setup-GPS: ======(%d)======> try to check if a NTP fix is available...\r",cntr);
     vTaskDelay(pdMS_TO_TICKS(1000));  
////    updateNTP();  
     update_NTP_GPS();
     if(ezTime_debug) debugA("   ezTime_setup-GPS: after events() NTP_time_status = %d \r",NTP_time_status);
     };

   if((cntr > 0) && (NTP_time_status == 2))Serial.printf("   ezTime_setup-GPS: NTP fix available: cntr = %d - NTP_time_status = %d \r", cntr, NTP_time_status );

   setInterval(0); // start ezTime events to service NTP_GPS sync  
   events();  // initialize or perform ezTime events   
}
